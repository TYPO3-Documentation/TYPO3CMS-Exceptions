# AGENTS.md — TYPO3 Exceptions

## Repo structure

```
Documentation/Exceptions/        # one page per TYPO3 exception code
Build/app/                       # console tool that creates missing page skeletons
Build/Exceptions/                # exception codes collected per Core tag (json + merged php array)
...
- `pre-commit run --all-files` — apply the whitespace hooks
  ...
  The hooks skip `*.json` and `Build/Exceptions/exceptions.php` on purpose:
  those files are generated and end without a trailing newline, so fixing them
  would just be reverted on the next generation run

## Page skeletons are generated, the explanations are not

A scheduled GitHub Action checks out TYPO3 Core tags, collects every
exception code, and creates a page from
`Build/app/Resources/Templates/default.rst` for any code that does not have
one yet. That is why commits such as
`[TASK] Created missing exception code page(s)` appear in the history. See
`README.rst` for the toolchain.

The **explanations on those pages are written by hand** — that is the actual
content of this manual. A page typically has one section per situation in
which the exception occurs, each with its cause and its solution.

## Always pin down the TYPO3 version

The same exception code can surface for different reasons in different TYPO3
versions, and a fix that is correct for one version can be wrong for
another — so a page must say which versions a described cause applies to.
Verify against the Core source of that version rather than from memory. When
a bug report or pull request does not name a version, ask for the full
version including the patch level.
