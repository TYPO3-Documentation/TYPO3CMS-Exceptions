...
::
      ...
      TYPO3\CMS\Core\Crypto\PasswordHashing\Argon2iPasswordHash is not available. If 
      the instance has just been upgraded, please log in to the standalone install
      tool at typo3/install.php to fix this. Follow the wiki link for more details.
...
-  **If access to install tool is not possible**: This could happen when
   ...
   -  open :file:`typo3conf/LocalConfiguration.php` change the password hashing
      to ``[FE][passwordHashing][className]`` and
