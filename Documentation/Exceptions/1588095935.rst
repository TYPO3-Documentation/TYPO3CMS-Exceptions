...
triggered by a failed check of a referrer header.
...
It may also happen if the webserver is not configured correctly to use a valid certificate.

Solution
--------

* Verify the values are correct for the variables:
...
* Verify the https connection has a valid certificate
